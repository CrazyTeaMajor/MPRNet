...
def my_collate_fn(x):
    img, label = list(zip(*x))
    
    if len(label) == 1:
        points = [{} for i in range(len(x))]
        for i in range(len(x)):
            points[i]['point'] = torch.Tensor(x[i][1])

        return torch.stack(img), points
    
    else:
        batch = len(label) * len(label[0])
        
        points = [{} for i in range(batch)]
        index = 0
        for i in range(len(label)):
            for j in range(len(label[0])):
                points[index]['point'] = torch.Tensor(label[i][j])
                index += 1
                
        return torch.cat(img, dim=0), points
